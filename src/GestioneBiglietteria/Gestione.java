...
public class Gestione {
    ...
    private Gestione(){
        stadio=new Stadio("Allianz Stadium",30);
        this.listaPartite= new HashMap<>();
    }
    public static Gestione getInstance(){
        if (tifoticket ==null)
    ...
    public boolean inserimentoNuovaPartita(String codice,LocalDateTime data, String avversario,String tipologia) throws PartitaErrore {
        Partita p=listaPartite.get(codice);
        if (p==null){
            if(!controllaSovrapposizione(data)){//se non si sovrappone con altre partite
                if(tipologia.equals("Campionato")||tipologia.equals("Coppa Italia")||tipologia.equals("Champions League") || tipologia.equals("Amichevole") || tipologia.equals("Supercoppa Italiana")){
    ...
    public Biglietto sostituzioneNominativo(String codiceBiglietto, String codicePartita, String nuovoNom, int nuovaEta) throws NominativoErrore {
        Partita pa= listaPartite.get(codicePartita);
